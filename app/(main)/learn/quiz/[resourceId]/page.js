...
export default function QuizPage() {
  ...
  const goNext = () => {
    if (currentQ + 1 < totalQ) setCurrentQ(i => i + 1);
  };
  const goBack = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Mark resource as complete if passed
          ...
          <div className="ij-card">
            ...
            <div className="ij-dots-row">
              {Array.from({ length: totalQ }).map((_, i) => (
                <div
                  key={i}
                  className={`ij-dot ${i === currentQ ? "active" : i < currentQ ? "done" : ""}`}
                />
              ))}
            </div>
  ...
  return (
    <div className="ij-results-card">
      ...
      <div className="ij-results-cta">
        {result.passed ? (
          <button className="ij-btn-primary" onClick={onBack}>
            Back to Roadmap ✓
          </button>
        ) : (
          <button className="ij-btn-secondary" onClick={() => window.location.reload()}>
            Retry Quiz ↺
          </button>
        )}
      </div>
