...
const C = {
  ...
  bg: "#F9F8F6", card: "#FFFFFF", success: "#16A34A", error: "#DC2626",
  muted: "#6B7280", border: "#E5E7EB", light: "#CCFBF1",
};
...
const Step1 = ({ data, set }) => {
  ...
  return (
    <div>
      ...
      <Field label="Current Address" required><Input placeholder="123 Mabini St, Quezon City" value={data.currentAddress} onChange={e => set({ ...data, currentAddress: e.target.value })} /></Field>
      <Field label="Permanent Address" required><Input placeholder="456 Rizal Ave, Cebu City" value={data.permanentAddress} onChange={e => set({ ...data, permanentAddress: e.target.value })} /></Field>
    ...
    <div>
      ...
      <Field label="Preferred Work Setup" required>
        <div style={{ display: "flex", gap: 8 }}>
          {workTypes.map(t => <button key={t} onClick={() => set({ ...data, workType: t })} style={{ flex: 1, padding: "8px 0", borderRadius: 8, fontSize: 12, fontWeight: 600, border: `1.5px solid ${data.workType === t ? C.accent : C.border}`, background: data.workType === t ? C.light : C.card, color: data.workType === t ? C.navy : C.muted, cursor: "pointer", transition: "all 0.15s" }}>{t}</button>)}
        </div>
      </Field>
      ...
      <Field label="Skills (select all that apply)">
        <ChipSelect options={skillOptions} selected={data.skills || []} onToggle={opt => { const c = data.skills || []; set({ ...data, skills: c.includes(opt) ? c.filter(s => s !== opt) : [...c, opt] }); }} />
      </Field>
      <Field label="Availability to Start">
        <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
          {availabilities.map(a => <button key={a} onClick={() => set({ ...data, availability: a })} style={{ padding: "5px 13px", borderRadius: 99, fontSize: 12, fontWeight: 600, border: `1.5px solid ${data.availability === a ? C.accent : C.border}`, background: data.availability === a ? C.light : C.card, color: data.availability === a ? C.navy : C.muted, cursor: "pointer", transition: "all 0.15s" }}>{a}</button>)}
        </div>
      </Field>
      <Field label="Career Goals">
        <textarea placeholder="e.g. I'm looking for a remote role where I can use my design skills..." rows={3} value={data.goals || ""} onChange={e => set({ ...data, goals: e.target.value })} style={{ ...inputBase, resize: "none", lineHeight: 1.5 }} onFocus={e => e.target.style.borderColor = C.accent} onBlur={e => e.target.style.borderColor = C.border} />
      </Field>
    </div>
  );
};
  ...
  return (
    <div>
      ...
      <Field label="Severity Level">
        <div style={{ display: "flex", gap: 8 }}>
          {severities.map(s => <button key={s} onClick={() => set({ ...data, severity: s })} style={{ flex: 1, padding: "8px 0", borderRadius: 8, fontSize: 12, fontWeight: 600, border: `1.5px solid ${data.severity === s ? C.accent : C.border}`, background: data.severity === s ? C.light : C.card, color: data.severity === s ? C.navy : C.muted, cursor: "pointer", transition: "all 0.15s" }}>{s}</button>)}
        </div>
      </Field>
      ...
      <Field label="Workplace Accommodations Needed">
        <ChipSelect options={accommodations} selected={data.accommodations || []} onToggle={opt => { const c = data.accommodations || []; set({ ...data, accommodations: c.includes(opt) ? c.filter(a => a !== opt) : [...c, opt] }); }} />
      </Field>
      <Field label="Additional notes">
        <textarea placeholder="e.g. I need an ergonomic chair and occasional breaks every 2 hours..." rows={3} value={data.disabilityNotes || ""} onChange={e => set({ ...data, disabilityNotes: e.target.value })} style={{ ...inputBase, resize: "none", lineHeight: 1.5 }} onFocus={e => e.target.style.borderColor = C.accent} onBlur={e => e.target.style.borderColor = C.border} />
      </Field>
      <label style={{ display: "flex", alignItems: "flex-start", gap: 10, cursor: "pointer" }}>
        <input type="checkbox" checked={data.consentSharing || false} onChange={e => set({ ...data, consentSharing: e.target.checked })} style={{ width: 14, height: 14, accentColor: C.accent, cursor: "pointer", marginTop: 2, flexShrink: 0 }} />
        <span style={{ fontSize: 12, color: C.muted, lineHeight: 1.6 }}>I consent to sharing my disability information with employers I apply to through InklusiJobs.</span>
      </label>
    </div>
  );
};
    ...
    <div>
      ...
      <Field label="Dashboard Theme">
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 8 }}>
          {themes.map(t => (
            <button key={t.id} onClick={() => set({ ...data, theme: t.id })} style={{ padding: "12px 8px", borderRadius: 10, border: `2px solid ${data.theme === t.id ? C.accent : C.border}`, cursor: "pointer", background: C.card, transition: "all 0.15s" }}>
      ...
      <Field label="Layout Density">
        <div style={{ display: "flex", gap: 8 }}>
          {layouts.map(l => <button key={l} onClick={() => set({ ...data, layout: l })} style={{ flex: 1, padding: "8px 0", borderRadius: 8, fontSize: 12, fontWeight: 600, border: `1.5px solid ${data.layout === l ? C.accent : C.border}`, background: data.layout === l ? C.light : C.card, color: data.layout === l ? C.navy : C.muted, cursor: "pointer", transition: "all 0.15s" }}>{l}</button>)}
        </div>
      </Field>
      <Field label="Default Widgets (pick up to 4)">
        <ChipSelect options={widgets} selected={data.widgets || []} max={4} onToggle={opt => { const c = data.widgets || []; set({ ...data, widgets: c.includes(opt) ? c.filter(w => w !== opt) : [...c, opt] }); }} />
      </Field>
      <Field label="Email Notifications">
        {[{ key: "newMatches", label: "New job match alerts" }, { key: "appUpdates", label: "Application status updates" }, { key: "interviewRemind", label: "Interview reminders" }, { key: "weeklyDigest", label: "Weekly job digest" }].map(({ key, label }) => (
...
export default function OnboardingPage() {
  ...
  const [s1, setS1] = useState({ firstName: "", lastName: "", age: "", currentAddress: "", permanentAddress: "", contactNumber: "", educationalAttainment: "" });
  ...
  const canNext = () => {
    ...
    if (step === 2) return s2.workType && s2.contractType && s2.contractType !== "Select contract type";
    return true;
  };
