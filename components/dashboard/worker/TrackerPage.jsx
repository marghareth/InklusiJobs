...
const C = {
  ink: '#1E1C19',
  inkMid: 'rgba(30,28,25,0.55)',
  inkFaint: 'rgba(30,28,25,0.32)',
  green: '#3D6B47',
  greenLight: '#6B8F71',
  greenBg: 'rgba(107,143,113,0.10)',
  greenBorder: 'rgba(107,143,113,0.24)',
  amber: '#7A5F20',
  amberBg: 'rgba(196,160,79,0.13)',
  amberBorder: 'rgba(196,160,79,0.28)',
  red: '#A84030',
  redBg: 'rgba(196,80,60,0.09)',
  redBorder: 'rgba(196,80,60,0.22)',
  blue: '#3A4F6E',
  blueBg: 'rgba(91,107,138,0.10)',
  blueBorder: 'rgba(91,107,138,0.22)',
  teal: '#2D5E4F',
  tealBg: 'rgba(45,94,79,0.09)',
  tealBorder: 'rgba(45,94,79,0.22)',
  sand: 'rgba(180,160,130,0.18)',
  sandBorder: 'rgba(180,160,130,0.22)',
  cardBg: 'rgba(255,255,255,0.75)',
  font: { display: '"Playfair Display", serif', body: '"Instrument Sans", sans-serif' },
};
...
───────────────────────────────────────────────────────────── */
const APP_STATUS = {
  applied:   { label: 'Applied',   color: C.blue,     bg: C.blueBg,   border: C.blueBorder,  step: 1, dot: '#5B6B8A' },
  screening: { label: 'Screening', color: C.amber,    bg: C.amberBg,  border: C.amberBorder, step: 2, dot: '#C4A04F' },
  interview: { label: 'Interview', color: C.teal,     bg: C.tealBg,   border: C.tealBorder,  step: 3, dot: '#3D8C70' },
  offer:     { label: 'Offer',     color: C.green,    bg: C.greenBg,  border: C.greenBorder, step: 4, dot: '#6B8F71' },
  rejected:  { label: 'Rejected',  color: C.red,      bg: C.redBg,    border: C.redBorder,   step: 0, dot: '#C4503C' },
  withdrawn: { label: 'Withdrawn', color: C.inkFaint, bg: C.sand,     border: C.sandBorder,  step: 0, dot: 'rgba(180,160,130,0.5)' },
};
...
───────────────────────────────────────────────────────────── */
function CircleRing({ pct, size = 120, stroke = 9, color = C.green, label, sublabel, animate = true }) {
  ...
  return (
    <div style={{ position: 'relative', width: size, height: size, flexShrink: 0 }}>
      <svg width={size} height={size} style={{ transform: 'rotate(-90deg)' }}>
        <circle cx={size/2} cy={size/2} r={r} fill="none" stroke="rgba(180,160,130,0.14)" strokeWidth={stroke} />
...
function StreakFlame({ streak, best }) {
  ...
  const hue   = Math.round(42 - intensity * 20);
  const sat   = Math.round(75 + intensity * 20);
  const light = Math.round(52 - intensity * 14);
    ...
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6, flexShrink: 0 }}>
      <div style={{ position: 'relative', width: size, height: size }}>
        ...
        <svg width={size} height={size} viewBox="0 0 56 56" fill="none" style={{ display: 'block' }}>
          ...
          />
          {streak > 0 && <ellipse cx="28" cy="34" rx="3.5" ry="5" fill="rgba(255,255,200,0.85)" />}
          <defs>
            <linearGradient id="flameGrad-outer" x1="28" y1="4" x2="28" y2="50" gradientUnits="userSpaceOnUse">
              <stop offset="0%"   stopColor={color} />
              <stop offset="60%"  stopColor={`hsl(${hue+10},${sat}%,${light+8}%)`} />
              <stop offset="100%" stopColor={`hsl(${hue-5},${sat-10}%,${light-8}%)`} stopOpacity="0.7" />
            </linearGradient>
            <linearGradient id="flameGrad-inner" x1="28" y1="18" x2="28" y2="42" gradientUnits="userSpaceOnUse">
              ...
              <stop offset="100%" stopColor={`hsl(${hue+5},${sat}%,${light+14}%)`} stopOpacity="0.6" />
            </linearGradient>
          </defs>
        </svg>
      </div>
...
function ActivityHeatmap({ activityData = {} }) {
  ...
  function cellColor(count, isFuture) {
    if (isFuture || count === 0) return 'rgba(180,160,130,0.12)';
    const t = count / maxCount;
    if (t < 0.25) return 'rgba(107,143,113,0.25)';
    if (t < 0.5)  return 'rgba(107,143,113,0.50)';
    if (t < 0.75) return 'rgba(107,143,113,0.75)';
    return '#3D6B47';
  }
      ...
      <div style={{ display: 'inline-block', minWidth: 'max-content' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 5, marginTop: 8, paddingLeft: 17 }}>
          ...
          {[0, 0.25, 0.5, 0.75, 1].map((t, i) => (
            <div key={i} style={{ width: 12, height: 12, borderRadius: 2.5, background: t === 0 ? 'rgba(180,160,130,0.12)' : `rgba(107,143,113,${t * 0.9 + 0.1})` }} />
          ))}
          <span style={{ fontFamily: C.font.body, fontSize: 10, color: C.inkFaint }}>More</span>
        </div>
      </div>
      {tooltip && (
        <div style={{ position: 'fixed', top: tooltip.y - 44, left: tooltip.x - 70, background: 'rgba(30,28,25,0.88)', color: '#fff', fontFamily: C.font.body, fontSize: 11, fontWeight: 600, padding: '5px 10px', borderRadius: 7, pointerEvents: 'none', zIndex: 999, whiteSpace: 'nowrap' }}>
        ...
        {weeks.map((w, i) => {
          ...
          return (
            <div key={i} onMouseEnter={() => setHovered(i)} onMouseLeave={() => setHovered(null)} style={{ position: 'relative', flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3, cursor: 'default' }}>
              <div style={{ width: '100%', height: h, borderRadius: '3px 3px 2px 2px', background: isLast ? 'linear-gradient(180deg,#6B8F71,#3D6B47)' : hovered === i ? 'rgba(107,143,113,0.55)' : 'rgba(107,143,113,0.28)', transition: 'background .15s', boxShadow: isLast ? '0 2px 8px rgba(107,143,113,0.30)' : 'none' }} />
              {hovered === i && (
                <div style={{ position: 'absolute', bottom: h + 6, background: 'rgba(30,28,25,0.85)', color: '#fff', fontFamily: C.font.body, fontSize: 10, fontWeight: 600, padding: '3px 7px', borderRadius: 5, whiteSpace: 'nowrap', zIndex: 10 }}>
                  {w.total} · {w.label}
                </div>
              )}
            </div>
          ...
          return (
            <div key={m} style={{ display: 'flex', alignItems: 'center', flex: i < milestones.length - 1 ? 1 : undefined }}>
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
                <div style={{
                  width: done ? 22 : isNext ? 18 : 14,
                  height: done ? 22 : isNext ? 18 : 14,
                  borderRadius: '50%',
                  background: done
                    ? 'linear-gradient(135deg,#6B8F71,#3D6B47)'
                    : isNext
                    ? `conic-gradient(#6B8F71 ${pct * 3.6}deg, rgba(180,160,130,0.25) 0deg)`
                    : 'rgba(180,160,130,0.18)',
                  border: done ? 'none' : isNext ? `2px solid rgba(107,143,113,0.5)` : `1.5px solid rgba(180,160,130,0.25)`,
                  display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0,
                  boxShadow: done ? '0 2px 6px rgba(107,143,113,0.30)' : 'none',
                }}>
                ...
                <span style={{ fontFamily: C.font.body, fontSize: 10, fontWeight: done ? 700 : 500, color: done ? C.green : isNext ? C.amber : C.inkFaint, whiteSpace: 'nowrap' }}>{m}d</span>
              </div>
              {i < milestones.length - 1 && (
                <div style={{ flex: 1, height: 2, background: done ? 'linear-gradient(90deg,#6B8F71,rgba(107,143,113,0.4))' : 'rgba(180,160,130,0.18)', marginBottom: 18, minWidth: 8 }} />
              )}
            </div>
          );
        })}
...
function StreakSection({ currentStreak, longestStreak, activityData = {} }) {
  ...
  const statItems = [
    ...
    { label: 'Best Streak',    value: `${longestStreak}d`, sub: 'personal record',  color: C.green,  border: C.greenBorder  },
    { label: 'Active Days',    value: totalActiveDays,     sub: 'total days',       color: C.blue,   border: C.blueBorder   },
    { label: 'Weekly Avg',     value: weeklyAvg,           sub: 'submissions / wk', color: C.teal,   border: C.tealBorder   },
  ];

  return (
    <div style={{ background: C.cardBg, backdropFilter: 'blur(14px)', WebkitBackdropFilter: 'blur(14px)', border: `1px solid ${C.sandBorder}`, borderRadius: 18, boxShadow: '0 2px 14px rgba(180,160,130,0.09)', overflow: 'hidden' }}>
      {/* Header */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 11, padding: '16px 22px', borderBottom: `1px solid ${C.sand}` }}>
        <div style={{ width: 32, height: 32, borderRadius: 9, background: C.amberBg, border: `1px solid ${C.amberBorder}`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
            <path d="M8 2C8 2 11 5 11 8.5C11 10.5 9.5 12 8 12C6.5 12 5 10.5 5 8.5C5 5 8 2 8 2Z" fill={C.amber} opacity="0.8"/>
            <path d="M8 7C8 7 9.5 8.5 9.5 10C9.5 10.83 8.83 11.5 8 11.5C7.17 11.5 6.5 10.83 6.5 10C6.5 8.5 8 7 8 7Z" fill="rgba(255,220,120,0.9)"/>
          </svg>
        </div>
      ...
      <div style={{ padding: '20px 22px', display: 'flex', flexDirection: 'column', gap: 20 }}>
        ...
        {/* TOP ROW: flame card + milestones + bars */}
        <div style={{ display: 'flex', gap: 22, alignItems: 'flex-start', flexWrap: 'wrap' }} className="streak-top-row">
          {/* Flame card */}
          <div style={{ padding: '16px 20px', background: C.amberBg, border: `1px solid ${C.amberBorder}`, borderRadius: 14, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12, flexShrink: 0, minWidth: 130 }}>
            <StreakFlame streak={currentStreak} best={longestStreak} />
            <div style={{ width: '100%', padding: '8px 10px', background: 'rgba(255,255,255,0.55)', borderRadius: 9, border: `1px solid ${C.amberBorder}`, textAlign: 'center' }}>
              <p style={{ fontFamily: C.font.display, fontSize: 16, fontWeight: 700, color: C.green, margin: 0 }}>{longestStreak}d</p>
            ...
            <div style={{ width: '100%' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                ...
                <span style={{ fontFamily: C.font.body, fontSize: 9.5, fontWeight: 700, color: C.amber }}>{longestStreak > 0 ? Math.round(currentStreak / longestStreak * 100) : 0}%</span>
              </div>
              <div style={{ height: 5, background: 'rgba(180,160,130,0.18)', borderRadius: 6, overflow: 'hidden' }}>
                <div style={{ height: '100%', width: `${longestStreak > 0 ? currentStreak / longestStreak * 100 : 0}%`, background: 'linear-gradient(90deg,#C4A04F,#8B6320)', borderRadius: 6, transition: 'width .6s cubic-bezier(0.4,0,0.2,1)' }} />
              </div>
            </div>
          </div>

          {/* Right: milestones + weekly bars */}
          <div style={{ flex: 1, minWidth: 240, display: 'flex', flexDirection: 'column', gap: 18 }}>
            ...
            <div style={{ height: 1, background: C.sand }} />
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 10 }} className="streak-stat-row">
          {statItems.map((s, i) => (
            <div key={i} style={{ padding: '12px 14px', background: 'rgba(255,255,255,0.55)', border: `1.5px solid ${s.border}`, borderRadius: 11 }}>
...
function FunnelViz({ applications }) {
  ...
  const stages = [
    ...
    { label: 'Active',     count: active,     pct: Math.round(active/total*100),      color: C.teal,  size: 100 },
    { label: 'Interviews', count: interviews, pct: Math.round(interviews/total*100),  color: C.amber, size: 74  },
    { label: 'Offers',     count: offers,     pct: Math.round(offers/total*100),      color: C.green, size: 52  },
  ];
  return (
        ...
        {stages.map((s, i) => (
          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ height: 3, background: 'rgba(180,160,130,0.15)', borderRadius: 4, marginTop: 3, overflow: 'hidden' }}>
...
function AppBadge({ status }) {
  const cfg = APP_STATUS[status] || { label: status, color: C.inkFaint, bg: C.sand, border: C.sandBorder, dot: 'rgba(180,160,130,0.5)' };
  return (
...
function Badge({ status }) {
  const cfg = {
    ...
    not_started:    { bg: C.sand,     color: C.inkFaint, border: C.sandBorder,  label: 'Not started' },
  }[status] || { bg: C.sand, color: C.inkFaint, border: C.sandBorder, label: status };
  return (
        ...
        return (
          <div key={s} style={{ display: 'flex', alignItems: 'center', flex: i < STEPS.length - 1 ? 1 : undefined }}>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 5 }}>
              <div style={{ width: current ? 13 : 9, height: current ? 13 : 9, borderRadius: '50%', background: isTerminal ? 'rgba(180,160,130,0.22)' : active ? cfg.dot : 'rgba(180,160,130,0.22)', boxShadow: current ? `0 0 0 4px ${cfg.bg}` : 'none' }} />
            ...
            {i < STEPS.length - 1 && <div style={{ flex: 1, height: 2, background: !isTerminal && step > i + 1 ? cfg.dot : 'rgba(180,160,130,0.18)', marginBottom: 18, marginTop: 2 }} />}
          </div>
        );
      })}
      {isTerminal && (
...
function AppTableRow({ app, expanded, onToggle }) {
  ...
  return (
    <>
      <tr onClick={onToggle} style={{ cursor: 'pointer', background: expanded ? 'rgba(107,143,113,0.04)' : 'transparent' }} className="app-tr">
        <td style={{ padding: '13px 16px 13px 20px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <div style={{ width: 32, height: 32, borderRadius: 8, flexShrink: 0, background: `hsl(${hue},25%,88%)`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: C.font.display, fontSize: 13, fontWeight: 700, color: `hsl(${hue},35%,38%)` }}>{(app.company || '?')[0].toUpperCase()}</div>
            <div>
      ...
      {expanded && (
        <tr>
          <td colSpan={6} style={{ padding: 0 }}>
            <div style={{ padding: '16px 20px 20px', background: 'rgba(107,143,113,0.03)', borderTop: `1px solid ${C.greenBorder}`, borderBottom: `1px solid ${C.greenBorder}` }}>
              ...
              {app.notes && (
                <div style={{ marginTop: 14, padding: '10px 14px', background: 'rgba(255,255,255,0.60)', border: `1px solid ${C.sandBorder}`, borderRadius: 9 }}>
                    ...
                    {app.history.map((h, i) => {
                      ...
                      return (
                        <div key={i} style={{ display: 'flex', alignItems: 'center' }}>
                          ...
                          {i < app.history.length - 1 && <div style={{ width: 24, height: 1.5, background: C.sand, marginBottom: 30 }} />}
                        </div>
                      );
                    })}
                  </div>
              ...
              {app.posting_url && (
                <div style={{ marginTop: 14 }}>
                  <a href={app.posting_url} target="_blank" rel="noopener noreferrer" style={{ fontFamily: C.font.body, fontSize: 12, fontWeight: 600, color: C.green, textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: 5, padding: '6px 13px', borderRadius: 8, border: `1.5px solid ${C.greenBorder}`, background: C.greenBg }}>
                    View Job Posting
                    <svg width="10" height="10" viewBox="0 0 10 10" fill="none"><path d="M2 8L8 2M8 2H4M8 2V6" stroke={C.green} strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round"/></svg>
                  </a>
                </div>
              )}
            </div>
...
function NotionCard({ isConnected, connectedAt, notionUrl }) {
  ...
  return (
    <div style={{ background: connected ? 'rgba(107,143,113,0.06)' : 'rgba(255,255,255,0.55)', border: connected ? `1.5px solid ${C.greenBorder}` : '1.5px dashed rgba(180,160,130,0.35)', borderRadius: 14, padding: '14px 20px', display: 'flex', alignItems: 'center', gap: 14, flexWrap: 'wrap' }}>
      <div style={{ width: 38, height: 38, borderRadius: 10, flexShrink: 0, background: connected ? C.greenBg : C.sand, border: `1px solid ${connected ? C.greenBorder : C.sandBorder}`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <svg width="18" height="18" viewBox="0 0 20 20" fill="none">
          <rect x="3.5" y="3.5" width="13" height="13" rx="2.5" stroke={connected ? C.green : C.inkFaint} strokeWidth="1.4"/>
          <path d="M7 7.5h6M7 10h6M7 12.5h4" stroke={connected ? C.green : C.inkFaint} strokeWidth="1.3" strokeLinecap="round"/>
        </svg>
      </div>
      ...
      {connected ? (
        <div style={{ display: 'flex', gap: 9, flexShrink: 0 }}>
          <span style={{ fontFamily: C.font.body, fontSize: 11, fontWeight: 700, background: C.greenBg, color: C.green, border: `1px solid ${C.greenBorder}`, borderRadius: 20, padding: '4px 12px' }}>Active</span>
          {notionUrl && <a href={notionUrl} target="_blank" rel="noopener noreferrer" style={{ fontFamily: C.font.body, fontSize: 12, fontWeight: 600, color: C.green, textDecoration: 'none', padding: '6px 14px', borderRadius: 9, border: `1.5px solid ${C.greenBorder}`, background: C.greenBg, whiteSpace: 'nowrap' }}>Open →</a>}
        </div>
      ) : (
        <button onClick={() => window.location.href = '/api/notion/connect'} style={{ padding: '9px 18px', borderRadius: 10, border: 'none', background: 'linear-gradient(135deg,#6B8F71,#4E7055)', color: '#fff', fontSize: 13, fontWeight: 700, fontFamily: C.font.body, cursor: 'pointer', boxShadow: '0 4px 14px rgba(107,143,113,0.28)', whiteSpace: 'nowrap', flexShrink: 0 }}>
          Connect
        </button>
      )}
...
───────────────────────────────────────────────────────────── */
function Divider() { return <div style={{ height: 1, background: C.sand }} />; }
    ...
    <>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, padding: '11px 0', flexWrap: 'wrap' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, minWidth: 0 }}>
          <div style={{ width: 7, height: 7, borderRadius: '50%', flexShrink: 0, background: status === 'approved' ? C.greenLight : status === 'pending' ? '#C4A04F' : status === 'rejected' ? '#C4503C' : 'rgba(180,160,130,0.35)' }} />
        ...
        <div style={{ display: 'flex', gap: 8, flexShrink: 0 }}>
          {score != null && <span style={{ fontFamily: C.font.body, fontSize: 12, fontWeight: 700, color: C.green, background: C.greenBg, border: `1px solid ${C.greenBorder}`, borderRadius: 8, padding: '2px 9px' }}>{score}</span>}
          <Badge status={status} />
        </div>
      </div>
      {!isLast && <Divider />}
...
function PhasePill({ status }) {
  const cfg = { active: { bg: C.greenBg, color: C.green, label: 'In Progress' }, completed: { bg: C.blueBg, color: C.blue, label: 'Completed' }, locked: { bg: C.sand, color: C.inkFaint, label: 'Locked' } }[status] || { bg: C.sand, color: C.inkFaint, label: status };
...
function PhaseBlock({ phase, challenges, isLast }) {
  ...
  return (
    <div style={{ marginBottom: isLast ? 0 : 12 }}>
      <div onClick={() => setOpen(!open)} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px', borderRadius: open ? '11px 11px 0 0' : 11, cursor: 'pointer', background: open ? C.greenBg : 'rgba(180,160,130,0.04)', border: `1px solid ${open ? C.greenBorder : C.sandBorder}`, borderBottom: open ? 'none' : undefined, transition: 'all .2s', userSelect: 'none' }}>
        <div style={{ width: 22, height: 22, borderRadius: 6, background: phase.status === 'locked' ? C.sand : C.greenBg, border: `1px solid ${phase.status === 'locked' ? C.sandBorder : C.greenBorder}`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
          {phase.status === 'completed'
            ? <svg width="11" height="11" viewBox="0 0 11 11" fill="none"><path d="M2 5.5L4.5 8 9 3" stroke={C.green} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>
            : phase.status === 'active'
            ? <svg width="10" height="10" viewBox="0 0 10 10" fill="none"><circle cx="5" cy="5" r="3" stroke={C.green} strokeWidth="1.5"/><circle cx="5" cy="5" r="1.2" fill={C.green}/></svg>
        ...
        {total > 0 && (
          <div style={{ width: 68, flexShrink: 0 }}>
            <div style={{ height: 4, background: 'rgba(180,160,130,0.18)', borderRadius: 10, overflow: 'hidden' }}>
              <div style={{ height: '100%', width: `${pct}%`, background: 'linear-gradient(90deg,#6B8F71,#4E7055)', borderRadius: 10 }} />
            </div>
      ...
      {open && (
        <div style={{ padding: challenges.length > 0 ? '0 16px' : '14px 16px', background: 'rgba(255,255,255,0.50)', border: `1px solid ${C.greenBorder}`, borderTop: 'none', borderRadius: '0 0 11px 11px' }}>
          {challenges.length > 0
  ...
  return (
    <>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, padding: '12px 0', flexWrap: 'wrap' }}>
        ...
        <div style={{ display: 'flex', gap: 8, flexShrink: 0 }}>
          {item.ai_score != null && <span style={{ fontFamily: C.font.body, fontSize: 13, fontWeight: 700, color: C.green, background: C.greenBg, border: `1px solid ${C.greenBorder}`, borderRadius: 8, padding: '2px 10px' }}>{item.ai_score}</span>}
  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 22 }} className="app-top-grid">
        <div style={{ padding: '18px 20px', background: 'rgba(255,255,255,0.60)', border: `1px solid ${C.sandBorder}`, borderRadius: 13 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
          {[
            { label: 'Total',          value: total,     sub: 'applications',   color: C.ink,   border: C.sandBorder  },
            { label: 'Active',         value: countBy('applied') + countBy('screening') + countBy('interview'), sub: 'in pipeline', color: C.teal, border: C.tealBorder },
            ...
            { label: 'Offer Rate',     value: total > 0 ? `${Math.round(offers/total*100)}%` : '—',    sub: `${offers} received`,  color: C.green, border: C.greenBorder },
          ].map((s, i) => (
            <div key={i} style={{ padding: '13px 15px', background: 'rgba(255,255,255,0.55)', border: `1px solid ${s.border}`, borderRadius: 11 }}>
      ...
      <div style={{ display: 'flex', gap: 9, marginBottom: 11, flexWrap: 'wrap', alignItems: 'center' }}>
        <div style={{ position: 'relative', flex: '1 1 180px', minWidth: 140 }}>
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search company or role…" style={{ width: '100%', paddingLeft: 30, paddingRight: 10, height: 34, borderRadius: 9, border: `1px solid ${C.sandBorder}`, background: 'rgba(255,255,255,0.65)', fontFamily: C.font.body, fontSize: 12.5, color: C.ink, outline: 'none' }} />
        </div>
        <select value={sort} onChange={e => setSort(e.target.value)} style={{ height: 34, padding: '0 10px', borderRadius: 9, border: `1px solid ${C.sandBorder}`, background: 'rgba(255,255,255,0.65)', fontFamily: C.font.body, fontSize: 12.5, color: C.ink, outline: 'none', cursor: 'pointer' }}>
      ...
      <div style={{ display: 'flex', gap: 5, marginBottom: 16, flexWrap: 'wrap' }}>
        {ALL_FILTERS.map(f => {
          ...
          return (
            <button key={f} onClick={() => setFilter(f)} style={{ fontFamily: C.font.body, fontSize: 11.5, fontWeight: 600, padding: '5px 12px', borderRadius: 20, border: active ? 'none' : `1px solid ${C.sandBorder}`, background: active ? 'linear-gradient(135deg,#6B8F71,#4E7055)' : 'rgba(255,255,255,0.55)', color: active ? '#fff' : C.inkMid, cursor: 'pointer', boxShadow: active ? '0 2px 8px rgba(107,143,113,0.22)' : 'none', display: 'inline-flex', gap: 5, alignItems: 'center' }}>
              {f}
              {count > 0 && <span style={{ background: active ? 'rgba(255,255,255,0.22)' : C.sand, borderRadius: 10, padding: '1px 6px', fontSize: 10 }}>{count}</span>}
            </button>
          );
        })}
      </div>
      ...
      {filtered.length === 0 ? (
        ...
        <div style={{ border: `1px solid ${C.sandBorder}`, borderRadius: 12, overflow: 'hidden' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ background: 'rgba(180,160,130,0.07)', borderBottom: `1px solid ${C.sand}` }}>
...
function Section({ icon, title, children, right }) {
  return (
    <div style={{ background: C.cardBg, backdropFilter: 'blur(14px)', WebkitBackdropFilter: 'blur(14px)', border: `1px solid ${C.sandBorder}`, borderRadius: 18, boxShadow: '0 2px 14px rgba(180,160,130,0.09)', overflow: 'hidden' }}>
      {title && (
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 22px', borderBottom: `1px solid ${C.sand}` }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 11 }}>
            <div style={{ width: 32, height: 32, borderRadius: 9, background: C.greenBg, border: `1px solid ${C.greenBorder}`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: C.green }}>{icon}</div>
...
export default function TrackerPage({
  ...
  submissions  = [],
  applications = [],
  activityData = {},  // { 'YYYY-MM-DD': submissionCount }
}) {
  ...
  return (
    <>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Playfair+Display:ital,wght@0,400;0,600;0,700;1,400&family=Instrument+Sans:wght@400;500;600;700&display=swap');
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .app-tr:hover { background: rgba(107,143,113,0.05) !important; }
        .app-tr + .app-tr { border-top: 1px solid rgba(180,160,130,0.13); }
        tr + tr:not(.exp-row) { border-top: 1px solid rgba(180,160,130,0.13); }
      ...
      <div style={{ padding: '30px 30px 48px', display: 'flex', flexDirection: 'column', gap: 18, maxWidth: 900, margin: '0 auto', width: '100%', fontFamily: C.font.body, color: C.ink }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
          <div>
            <p style={{ fontFamily: C.font.body, fontSize: 11, fontWeight: 700, color: C.greenLight, textTransform: 'uppercase', letterSpacing: '1.2px', margin: '0 0 5px' }}>Dashboard</p>
          ...
          <div style={{ display: 'flex', gap: 7, flexWrap: 'wrap' }}>
            <span style={{ fontFamily: C.font.body, fontSize: 11.5, fontWeight: 600, color: C.green, background: C.greenBg, border: `1px solid ${C.greenBorder}`, borderRadius: 20, padding: '5px 13px' }}>{approvedCount}/{totalChallenges} challenges</span>
        ...
        {/* TAB BAR */}
        <div style={{ display: 'flex', gap: 2, background: 'rgba(180,160,130,0.09)', border: `1px solid ${C.sandBorder}`, borderRadius: 12, padding: '3px', width: 'fit-content' }}>
          {TABS.map(t => (
            <button key={t.id} onClick={() => setTab(t.id)} style={{ fontFamily: C.font.body, fontSize: 13, fontWeight: 600, padding: '7px 16px', borderRadius: 9, border: 'none', background: tab === t.id ? 'rgba(255,255,255,0.92)' : 'transparent', color: tab === t.id ? C.ink : C.inkFaint, cursor: 'pointer', boxShadow: tab === t.id ? '0 1px 4px rgba(180,160,130,0.18)' : 'none', transition: 'all .15s' }}>{t.label}</button>
          ))}
        </div>
        ...
        {tab === 'overview' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            {/* Hero row: challenge ring + stat tiles */}
            <div style={{ display: 'grid', gridTemplateColumns: 'auto 1fr', gap: 16 }} className="hero-grid">
              <div style={{ padding: '22px 24px', background: C.cardBg, backdropFilter: 'blur(14px)', border: `1px solid ${C.sandBorder}`, borderRadius: 18, boxShadow: '0 2px 14px rgba(180,160,130,0.09)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 12 }}>
                <CircleRing pct={overallPct} size={136} stroke={11} color={C.green} label="completed" sublabel={`${approvedCount}/${totalChallenges}`} animate />
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10 }} className="stat-grid-6">
                {[
                  ...
                  { label: 'Approved',     value: stats.totalApproved,                       sub: 'verified',      color: C.green, border: C.greenBorder },
                  ...
                  { label: 'Score',        value: Number(stats.verificationScore).toFixed(1), sub: 'verification', color: C.teal,  border: C.tealBorder  },
                  { label: 'Applications', value: applications.length,                        sub: 'tracked',      color: C.blue,  border: C.blueBorder  },
                ].map((s, i) => (
                  <div key={i} style={{ padding: '13px 14px', background: 'rgba(255,255,255,0.65)', border: `1.5px solid ${s.border}`, borderRadius: 12 }}>
                    <p style={{ fontFamily: C.font.display, fontSize: 21, fontWeight: 700, color: s.color, margin: 0, lineHeight: 1 }}>{s.value}</p>
                    <p style={{ fontFamily: C.font.body, fontSize: 10, fontWeight: 700, color: C.inkFaint, margin: '4px 0 1px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>{s.label}</p>
                    <p style={{ fontFamily: C.font.body, fontSize: 10.5, color: C.inkFaint, margin: 0 }}>{s.sub}</p>
                  </div>
            ...
            {applications.length > 0 && (
              <Section
                icon={<svg width="15" height="15" viewBox="0 0 16 16" fill="none"><rect x="2" y="2" width="12" height="12" rx="2.5" stroke={C.green} strokeWidth="1.4"/><path d="M5 6h6M5 9h4" stroke={C.green} strokeWidth="1.3" strokeLinecap="round"/></svg>}
                title="Application Snapshot"
                right={<button onClick={() => setTab('applications')} style={{ fontFamily: C.font.body, fontSize: 12, fontWeight: 600, color: C.green, background: C.greenBg, border: `1px solid ${C.greenBorder}`, borderRadius: 8, padding: '5px 12px', cursor: 'pointer' }}>View all →</button>}
              >
            ...
            {/* Challenge breakdown */}
            <Section
              icon={<svg width="15" height="15" viewBox="0 0 16 16" fill="none"><path d="M2 14V6l5-4 5 4v8" stroke={C.green} strokeWidth="1.4" strokeLinejoin="round"/><rect x="6" y="9" width="4" height="5" rx="0.8" stroke={C.green} strokeWidth="1.2"/></svg>}
              title="Challenge Breakdown"
            >
              <div style={{ display: 'flex', gap: 18, marginBottom: 12, flexWrap: 'wrap' }}>
                {[
                  { dot: C.greenLight,               label: `${stats.totalApproved} Approved`  },
                  { dot: '#C4A04F',                  label: `${challenges.filter(c=>c.status==='pending').length} Pending` },
                  { dot: '#C4503C',                  label: `${stats.totalRejected} Rejected`   },
                  { dot: 'rgba(180,160,130,0.45)',   label: `${challenges.filter(c=>c.status==='not_started').length} Not started` },
                ].map((item, i) => (
              ...
              <div style={{ height: 9, background: 'rgba(180,160,130,0.13)', borderRadius: 10, overflow: 'hidden' }}>
                <div style={{ height: '100%', width: `${overallPct}%`, background: 'linear-gradient(90deg,#6B8F71,#4E7055)', borderRadius: 10, transition: 'width .6s cubic-bezier(0.4,0,0.2,1)' }} />
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 8 }}>
                ...
                <p style={{ fontFamily: C.font.display, fontSize: 15, fontWeight: 700, color: C.green, margin: 0 }}>{overallPct}%</p>
              </div>
            </Section>
        ...
        {tab === 'roadmap' && (
          <Section
            icon={<svg width="15" height="15" viewBox="0 0 16 16" fill="none"><circle cx="3" cy="13" r="1.5" fill={C.green}/><circle cx="8" cy="3" r="1.5" fill={C.green}/><circle cx="13" cy="13" r="1.5" fill={C.green}/><path d="M3 13C4 8 6 3 8 3M8 3C10 3 12 8 13 13" stroke={C.green} strokeWidth="1.3" strokeLinecap="round"/></svg>}
            title="My Roadmap"
          >
            {phases.length === 0
        ...
        {tab === 'submissions' && (
          <Section
            icon={<svg width="15" height="15" viewBox="0 0 16 16" fill="none"><path d="M3 4h10M3 7.5h7M3 11h5" stroke={C.green} strokeWidth="1.4" strokeLinecap="round"/></svg>}
 ...
 *
 * activityData prop:
 *   { 'YYYY-MM-DD': submissionCount, ... }
 *   e.g. { '2025-01-15': 2, '2025-01-16': 1, '2025-01-18': 3 }
 *
 ...
 *     const activityData = data.reduce((acc, row) => {
 *       const day = row.submitted_at.split('T')[0]
 *       acc[day] = (acc[day] || 0) + 1
 *       return acc
 *     }, {})
 *
 * applications prop:
 *   [{
 *     id, company, role, location, status, applied_at,
 *     last_update_at, interview_at, followup_at, salary,
 *     notes, posting_url,
 *     history: [{ status, date, note? }]
 *   }]
 *
 * Notion schema additions (Applications DB):
 *   Company (Title), Role (Text), Location (Text),
 *   Status (Select), Applied Date, Last Update, Interview Date,
 *   Follow-up Date, Salary (Text), Notes (Text), URL (URL),
 *   Worker (Relation), Status History (Rich Text — JSON)
 * ═══════════════════════════════════════════════════════════
 */