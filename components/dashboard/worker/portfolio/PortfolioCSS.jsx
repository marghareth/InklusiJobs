...
/* ═══════════════════════════════════════
   DESIGN TOKENS — InklusiJobs palette
═══════════════════════════════════════ */
.pf-root {
  --cream:   #F2EFE9;
  --cream2:  #F7F5F0;
  ...
  --forest2: #3D6B1F;
  --forest3: #4A7C59;
  --fdim:    rgba(45,80,22,.09);
  --fbdr:    rgba(45,80,22,.25);
  --fmed:    rgba(45,80,22,.45);
  --gold:    #7B6226;
  --gdim:    rgba(123,98,38,.10);
  --gbdr:    rgba(123,98,38,.25);
  --sage:    #5C8A56;
  --sdim:    rgba(92,138,86,.09);
  --sbdr:    rgba(92,138,86,.24);
  --sh:      rgba(28,43,26,.08);
  --sh2:     rgba(28,43,26,.16);
  --sh3:     rgba(28,43,26,.28);
  --red:     #A83C3C;
...
.pf-banner-text { font-size: .82rem; line-height: 1.5; }
.pf-banner-prev { display: block; font-size: .72rem; opacity: .7; margin-top: .06rem; }
.pf-banner-dismiss {
...
.pf-page-sub { font-size: .86rem; color: var(--ink2); }
...
.pf-btn-outline:hover { background: var(--fdim); }
.pf-btn-solid {
  background: var(--forest); border: none; color: #fff;
  box-shadow: 0 3px 12px var(--sh2);
}
.pf-btn-solid:hover { background: var(--forest2); transform: translateY(-1px); box-shadow: 0 5px 18px var(--sh2); }
.pf-btn-copied { background: var(--sage) !important; }
...
.pf-av-col {
  ...
  background: var(--fdim); border-right: 1px solid var(--fbdr);
  min-width: 160px; flex-shrink: 0;
}
.pf-av-wrap { position: relative; }
.pf-av {
  width: 82px; height: 82px; border-radius: 50%;
  background: linear-gradient(145deg, var(--forest), var(--forest3));
  color: #fff; font-size: 1.5rem; font-weight: 800;
  display: flex; align-items: center; justify-content: center;
  border: 3px solid rgba(45,80,22,.2);
  box-shadow: 0 6px 20px rgba(45,80,22,.2);
}
.pf-av-status {
  position: absolute; bottom: 3px; right: 3px;
  width: 15px; height: 15px; border-radius: 50%;
  background: #52c870; border: 2.5px solid var(--white);
}
.pf-av-since {
  display: flex; align-items: center; gap: .3rem;
  font-size: .68rem; color: var(--forest2); font-weight: 600;
...
.pf-field-badge {
  ...
  background: var(--fdim); color: var(--forest);
  border: 1px solid var(--fbdr); font-size: .74rem; font-weight: 700;
}

.pf-hl-row { display: flex; align-items: center; gap: .4rem; margin-bottom: 1rem; }
.pf-headline { font-size: 1rem; color: var(--ink2); font-weight: 500; flex: 1; }
.pf-hl-edit-btn {
...
.pf-hl-edit-btn:hover { border-color: var(--forest); color: var(--forest); }
.pf-hl-edit-row { margin-bottom: .8rem; }
.pf-hl-inp {
  width: 100%; font-family: 'Plus Jakarta Sans', sans-serif; font-size: .9rem; font-weight: 500;
  padding: .35rem .7rem; border: 1.5px solid var(--forest); border-radius: 8px;
...
.pf-detail-pwd { color: var(--forest3); font-weight: 600; }
.pf-detail-code { font-family: 'DM Mono', monospace; font-size: .88rem; color: var(--ink2); }

/* Stats column */
.pf-stats-col {
  display: flex; flex-direction: column; gap: .65rem;
  padding: 1.75rem 1.75rem; border-left: 1px solid var(--bdr);
...
.stat-pill-n {
  ...
  letter-spacing: -.04em; color: var(--acc, var(--forest));
}
.stat-pill-l {
  display: block; font-size: .72rem; color: var(--ink2);
  font-weight: 500; margin-top: .06rem;
}
...
.pf-update-strip {
  ...
  border-radius: 9px; font-size: .78rem; color: var(--gold);
}
...
.pf-tab-active { color: var(--forest); border-bottom-color: var(--forest); }
...
.pf-card {
  ...
  box-shadow: 0 1px 6px var(--sh); transition: box-shadow .2s;
}
...
.pf-card-title { font-size: .95rem; font-weight: 700; color: var(--ink); }
.pf-card-note  { font-size: .72rem; color: var(--ink3); }
.pf-card-action {
  display: flex; align-items: center; gap: .28rem;
  background: none; border: 1px solid var(--bdr2); border-radius: 7px;
  padding: .26rem .68rem; font-family: 'Plus Jakarta Sans', sans-serif;
  font-size: .73rem; font-weight: 600; color: var(--ink2); cursor: pointer;
  transition: all .18s;
}
.pf-card-action:hover { border-color: var(--forest); color: var(--forest); }
...
.pf-bio-inp {
  ...
  padding: .65rem .88rem; border: 1.5px solid var(--forest);
  border-radius: 10px; background: var(--cream2); color: var(--ink);
  outline: none; resize: vertical;
}

/* Challenge summary boxes */
.pf-ch-sum-boxes { display: grid; grid-template-columns: 1fr 1fr 1fr; gap: .55rem; margin-bottom: .9rem; }
.pf-sum-box {
  padding: .7rem .5rem; border-radius: 10px; text-align: center;
  border: 1px solid transparent;
}
.pf-sum-green { background: var(--fdim); border-color: var(--fbdr); }
.pf-sum-gold  { background: var(--gdim); border-color: var(--gbdr); }
.pf-sum-pts   { background: var(--cream2); border-color: var(--bdr); }
.pf-sum-n { display: block; font-size: 1.4rem; font-weight: 800; letter-spacing: -.04em; }
.pf-sum-green .pf-sum-n { color: var(--forest); }
.pf-sum-gold  .pf-sum-n { color: var(--gold); }
.pf-sum-pts   .pf-sum-n { color: var(--ink); }
...
.pf-ch-mini-row  { display: flex; align-items: center; gap: .5rem; font-size: .78rem; padding: .28rem 0; border-top: 1px solid var(--bdr); }
.pf-ch-mini-row:first-child { border-top: none; }
.pf-mini-dot { width: 8px; height: 8px; border-radius: 50%; flex-shrink: 0; }
.pf-mini-dot-done  { background: var(--forest); }
.pf-mini-dot-going { background: var(--gold); }
...
.sk-name { font-size: .83rem; font-weight: 600; color: var(--ink); }
.sk-tag  {
  padding: .1rem .42rem; border-radius: 999px;
  background: var(--cream3); border: 1px solid var(--bdr);
  font-size: .63rem; font-weight: 600; color: var(--ink3);
}
.sk-pct { font-size: .74rem; font-weight: 700; color: var(--forest); font-family: 'DM Mono', monospace; }
...
.pf-link-pill {
  ...
  border: 1px solid var(--fbdr); background: var(--fdim);
  color: var(--forest); font-size: .78rem; font-weight: 600;
  text-decoration: none; transition: all .18s;
}
.pf-link-pill:hover { background: var(--forest); color: #fff; }
.pf-empty-hint { font-size: .78rem; color: var(--ink3); font-style: italic; }
.pf-info-note {
  display: flex; align-items: center; gap: .5rem;
  margin-top: 1rem; padding: .6rem .95rem;
  background: var(--fdim); border: 1px solid var(--fbdr);
  border-radius: 9px; font-size: .78rem; color: var(--forest);
}
...
.pf-ch-ss-done  { background: var(--fdim); border-color: var(--fbdr); }
.pf-ch-ss-going { background: var(--gdim); border-color: var(--gbdr); }
.pf-ch-ss-pts   { background: var(--white); border-color: var(--bdr); }
.pf-ch-ss-n {
  display: block; font-size: 1.6rem; font-weight: 800; letter-spacing: -.04em;
}
.pf-ch-ss-done  .pf-ch-ss-n { color: var(--forest); }
.pf-ch-ss-going .pf-ch-ss-n { color: var(--gold); }
.pf-ch-ss-pts   .pf-ch-ss-n { color: var(--ink); }
...
.ch-done .ch-stripe  { background: var(--forest); }
.ch-going .ch-stripe { background: var(--gold); }
...
.ch-sbadge-done  { background: var(--fdim); color: var(--forest); border: 1px solid var(--fbdr); }
.ch-sbadge-going { background: var(--gdim); color: var(--gold); border: 1px solid var(--gbdr); }
.ch-title { font-size: .9rem; font-weight: 700; color: var(--ink); line-height: 1.35; }
.ch-pts-wrap {
  display: flex; align-items: center; gap: .28rem;
...
.ch-field-tag {
  padding: .1rem .5rem; background: var(--cream2); border: 1px solid var(--bdr);
...
.cs-sec-title { font-size: .9rem; font-weight: 700; color: var(--ink); }
.cs-title-inp {
...
.cs-del-btn {
  display: flex; align-items: center; gap: .28rem;
  background: none; border: 1px solid var(--bdr2); border-radius: 7px;
  padding: .24rem .65rem; font-family: 'Plus Jakarta Sans', sans-serif;
...
.cs-link-row {
  ...
  padding: .42rem .75rem; background: var(--white);
  border: 1px solid var(--bdr); border-radius: 9px;
}
.cs-link-ico { color: var(--forest); display: flex; align-items: center; flex-shrink: 0; }
.cs-link-name {
  font-size: .83rem; font-weight: 700; color: var(--forest);
...
.cs-link-form {
  ...
  margin-top: .45rem; padding: .85rem; background: var(--white);
  border: 1.5px solid var(--fbdr); border-radius: 10px;
}
.cs-inp {
  padding: .44rem .75rem; border: 1px solid var(--bdr2); border-radius: 8px;
  background: var(--cream2); color: var(--ink);
  font-family: 'Plus Jakarta Sans', sans-serif; font-size: .82rem; outline: none;
  transition: border-color .16s;
}
.cs-inp:focus { border-color: var(--forest); }
.cs-link-form-btns { display: flex; gap: .45rem; }
.cs-confirm-btn, .cs-cancel-btn {
  display: flex; align-items: center; gap: .3rem;
  padding: .4rem .95rem; border-radius: 8px;
  font-family: 'Plus Jakarta Sans', sans-serif; font-size: .78rem; font-weight: 700; cursor: pointer;
}
.cs-confirm-btn { background: var(--forest); border: none; color: #fff; }
.cs-confirm-btn:hover { background: var(--forest2); }
...
.cs-add-link-btn {
  ...
  padding: .4rem .9rem; font-family: 'Plus Jakarta Sans', sans-serif;
  font-size: .78rem; font-weight: 600; color: var(--ink3); cursor: pointer;
  transition: all .16s; margin-top: .35rem;
}
.cs-add-link-btn:hover { border-color: var(--forest); color: var(--forest); }

/* Text */
.cs-text-view { font-size: .84rem; color: var(--ink2); line-height: 1.7; white-space: pre-line; }
.cs-textarea {
  width: 100%; font-family: 'Plus Jakarta Sans', sans-serif; font-size: .84rem;
  line-height: 1.65; padding: .65rem .88rem;
  border: 1.5px solid var(--bdr2); border-radius: 10px;
  background: var(--white); color: var(--ink); outline: none; resize: vertical;
  transition: border-color .16s;
}
.cs-textarea:focus { border-color: var(--forest); }

/* Image */
.cs-img-area { display: flex; flex-direction: column; gap: .65rem; }
.cs-img-drop {
  border: 2px dashed var(--bdr); border-radius: 13px;
  padding: 2.5rem 1.5rem; cursor: pointer;
  display: flex; flex-direction: column; align-items: center; gap: .6rem;
  color: var(--ink3); transition: all .18s; background: var(--white);
}
.cs-img-drop:hover { border-color: var(--forest); color: var(--forest); }
...
.cs-add-row {
  ...
  padding: .85rem 1rem; border: 1.5px dashed var(--bdr);
...
.cs-add-btn:hover { border-color: var(--forest); color: var(--forest); background: var(--fdim); }
...
.ep-overlay {
  ...
  background: rgba(28,43,26,.55); backdrop-filter: blur(6px);
...
.ep-topbar {
  display: flex; align-items: center; justify-content: space-between;
  padding: .9rem 1.6rem; background: var(--forest); color: #fff;
}
.ep-topbar-inner { display: flex; align-items: center; gap: 1rem; }
.ep-mode-pill {
  display: flex; align-items: center; gap: .35rem;
  padding: .25rem .8rem; border-radius: 999px;
  background: rgba(255,255,255,.22); font-size: .72rem; font-weight: 800; letter-spacing: .05em;
}
.ep-mode-note { font-size: .77rem; opacity: .75; }
.ep-close {
...
.ep-hero-bg {
  ...
  background: linear-gradient(135deg, var(--fdim) 0%, transparent 60%);
}
...
.ep-av {
  ...
  display: flex; align-items: center; justify-content: center;
  box-shadow: 0 6px 18px rgba(45,80,22,.25);
}
.ep-hero-info { flex: 1; }
.ep-name { font-size: 1.45rem; font-weight: 800; color: var(--ink); letter-spacing: -.03em; margin-bottom: .2rem; }
.ep-headline { font-size: .9rem; color: var(--forest); font-weight: 600; margin-bottom: .55rem; }
.ep-contact-row {
...
.ep-tag-pwd   { background: var(--fdim); color: var(--forest); border: 1px solid var(--fbdr); }
.ep-tag-field { background: var(--cream2); color: var(--ink2); border: 1px solid var(--bdr); }
.ep-tag-id    { background: var(--gdim); color: var(--gold); border: 1px solid var(--gbdr); font-family: 'DM Mono', monospace; }
...
.ep-stat-n { display: block; font-size: 1.4rem; font-weight: 800; color: var(--forest); letter-spacing: -.04em; }
...
.ep-ch-row {
  ...
  padding: .5rem .8rem; background: var(--cream2); border: 1px solid var(--bdr); border-radius: 9px;
  font-size: .82rem;
}
.ep-ch-check { color: var(--forest); display: flex; align-items: center; flex-shrink: 0; }
.ep-ch-title { flex: 1; color: var(--ink); font-weight: 600; min-width: 180px; }
.ep-ch-tag   { padding: .1rem .45rem; background: var(--white); border: 1px solid var(--bdr); border-radius: 5px; font-size: .65rem; font-weight: 600; color: var(--ink2); flex-shrink: 0; }
.ep-ch-date  { font-size: .7rem; color: var(--ink3); font-family: 'DM Mono', monospace; flex-shrink: 0; }
.ep-ch-pts   { display: flex; align-items: center; gap: .22rem; font-size: .72rem; color: var(--ink2); font-family: 'DM Mono', monospace; flex-shrink: 0; margin-left: auto; }
.ep-ch-pts svg { color: var(--gold); }

/* Footer */
.ep-footer {
  display: flex; align-items: center; justify-content: center; gap: .45rem;
  padding: .9rem 1.75rem; font-size: .73rem; color: var(--ink3);
  background: var(--cream2); border-top: 1px solid var(--bdr);
}
...
@media(max-width:800px){
  ...
  .pf-av-col{flex-direction:row;padding:1rem 1.25rem;border-right:none;border-bottom:1px solid var(--fbdr);width:100%;}
