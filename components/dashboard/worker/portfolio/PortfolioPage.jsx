...
export default function PortfolioPage() {
  ...
  const [sections, setSections]       = useState(() => getInitSections(USER.name));
  ...
  const handleShare = () => {
    ...
    navigator.clipboard?.writeText(`https://inklusijobs.ph/portfolio/${slug}`).then(() => {
      setCopied(true);
    ...
    <>
      ...
      <div className="pf-root">
          ...
          <div className="pf-info-col">
            ...
            {editHL ? (
              <div className="pf-hl-edit-row">
                <input className="pf-hl-inp" value={headline} autoFocus
            ...
            <div className="pf-details-grid">
              <div className="pf-detail"><span className="pf-detail-k">Email</span><span className="pf-detail-v">{USER.email}</span></div>
              <div className="pf-detail"><span className="pf-detail-k">Phone</span><span className="pf-detail-v">{USER.phone}</span></div>
              <div className="pf-detail"><span className="pf-detail-k">Age</span><span className="pf-detail-v">{USER.age}</span></div>
              <div className="pf-detail"><span className="pf-detail-k">Address</span><span className="pf-detail-v">{USER.address}</span></div>
              <div className="pf-detail"><span className="pf-detail-k">Disability</span><span className="pf-detail-v pf-detail-pwd">♿ {USER.disability}</span></div>
              <div className="pf-detail"><span className="pf-detail-k">PWD ID No.</span><span className="pf-detail-v pf-detail-code">{USER.pwdIdNo}</span></div>
            </div>
          </div>
        ...
        <div className="pf-update-strip">
          <IcInfo />
          <span>Last updated <strong>{USER.lastUpdated}</strong>
            {USER.prevUpdate !== "—" && <span className="pf-update-prev"> · Previous: {USER.prevUpdate}</span>}
          </span>
        </div>

        <div className="pf-tabs-bar">
          {TABS.map(t => (
            <button key={t.id} className={`pf-tab ${tab === t.id ? "pf-tab-active" : ""}`} onClick={() => setTab(t.id)}>
              {t.label}
            </button>
          ))}
        </div>
            ...
            <div className="pf-overview-grid">
              ...
              <div className="pf-card">
                ...
                <div className="pf-ch-sum-boxes">
                  <div className="pf-sum-box pf-sum-green"><span className="pf-sum-n">{done.length}</span><span className="pf-sum-l">Completed</span></div>
                  <div className="pf-sum-box pf-sum-gold"><span className="pf-sum-n">{ongoing.length}</span><span className="pf-sum-l">Ongoing</span></div>
                  <div className="pf-sum-box pf-sum-pts"><span className="pf-sum-n">{totalPts.toLocaleString()}</span><span className="pf-sum-l">Points</span></div>
                </div>
          ...
          {tab === "challenges" && (
            <>
              <div className="pf-ch-strip">
                <div className="pf-ch-strip-stat pf-ch-ss-done"><span className="pf-ch-ss-n">{done.length}</span><span className="pf-ch-ss-l">Completed</span></div>
                <div className="pf-ch-strip-stat pf-ch-ss-going"><span className="pf-ch-ss-n">{ongoing.length}</span><span className="pf-ch-ss-l">In Progress</span></div>
                <div className="pf-ch-strip-stat pf-ch-ss-pts"><span className="pf-ch-ss-n">{totalPts.toLocaleString()}</span><span className="pf-ch-ss-l">Points Earned</span></div>
              </div>
              {ongoing.length > 0 && (
                <>
        ...
        {showPreview && (
          <EmployerPreview bio={bio} headline={headline} sections={sections} onClose={() => setShowPreview(false)} />
        )}
