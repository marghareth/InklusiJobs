    ...
    <div className="cs-root">
      {sections.map(sec => (
        <div key={sec.id} className="cs-sec">
              ...
              {!readOnly && (
                addingLink === sec.id ? (
                  <div className="cs-link-form">
                    ...
                    <input 
                      className="cs-inp" 
                      placeholder="URL (e.g. facebook.com/you)" 
          ...
          {sec.type === "text" && (
            readOnly ? (
              <p className="cs-text-view">{sec.content || <em style={{ color: "#9CA88F" }}>—</em>}</p>
            ) : (
              <textarea 
      ...
      <input 
        ...
        onChange={e => {
          ...
          r.onload = ev => {
            patch(uploadId, { src: ev.target.result });
            setUploadId(null);
          };
      ...
      {!readOnly && (
        <div className="cs-add-row">
          ...
          <button className="cs-add-btn" onClick={() => addSection("links")}>
            <IcLink /> Links
          </button>
          <button className="cs-add-btn" onClick={() => addSection("text")}>
            <IcTextB /> Text block
          </button>
          <button className="cs-add-btn" onClick={() => addSection("image")}>
            <IcImage /> Image
          </button>
        </div>
      )}
