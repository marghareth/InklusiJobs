...
/**
 * Called by the quiz page after a user completes an assessment.
 * Syncs the result into the master state so Tracker updates live.
 *
 * @param {object}  resource    full resource object from learn-content.js
 * @param {string}  resourceId  resource.id e.g. "smm_r1"
 * @param {number}  score       0–100 overall quiz score
 * @param {boolean} passed      true if score >= 70
 */
export function syncQuizToTracker({ resource, resourceId, score, passed }) {
  // ✅ Add debug logging
  console.log("syncQuizToTracker called with:", { resource, resourceId, score, passed });
  
  // ✅ Validate inputs
  if (!resource || !resourceId) {
    console.error("syncQuizToTracker: Missing required parameters");
    return;
  }

  const state = storage.get();
  console.log("Current state before sync:", state);
  
  const challenges = state.challenges || [];
  const existing = challenges.find(c => c.id === resourceId);
  const status = passed ? 'approved' : 'needs_revision';

  // ✅ Ensure resource has all required fields with defaults
  const safeResource = {
    title: resource.title || 'Untitled Quiz',
    phaseNumber: resource.phaseNumber || 1,
    estimatedHours: resource.estimatedHours || 1,
    skillMapped: resource.skillMapped || 'General Skills',
    ...resource
  };
  ...
  const entry = {
    ...
    description: 'Quiz: ' + safeResource.title,
    phase_id: 'phase_' + safeResource.phaseNumber,
    phase_name: 'Phase ' + safeResource.phaseNumber + (safeResource.skillMapped ? ' — ' + safeResource.skillMapped : ''),
    status,
    ai_score: passed ? score : (existing?.ai_score ?? null),
    submittedAt: new Date().toISOString(),
    estimatedHours: safeResource.estimatedHours,
    portfolioWorthy: passed,
  };

  console.log("Creating challenge entry:", entry);

  const updatedChallenges = existing
    ? challenges.map(c => c.id === resourceId ? { ...c, ...entry } : c)
    : [...challenges, entry];

  // ✅ Always log submission, even if not passed
  storage.logSubmission({
    challengeId: resourceId,
    challengeTitle: safeResource.title,
    status,
    ai_score: score,
  });

  const phases = state.tracker?.phases || [];
  const phaseId = 'phase_' + safeResource.phaseNumber;
  const phaseExists = phases.some(p => p.id === phaseId);
  
  const updatedPhases = phaseExists ? phases : [
    ...phases,
    {
      id: phaseId,
      phase_name: 'Phase ' + safeResource.phaseNumber + (safeResource.skillMapped ? ' — ' + safeResource.skillMapped : ''),
      phaseNumber: safeResource.phaseNumber,
      status: 'active',
    },
  ];
  ...
  const updateData = {
    ...
    tracker: { phases: updatedPhases },
  };
  
  console.log("Updating storage with:", updateData);
  ...
  setTimeout(() => {
    ...
    console.log("State after sync:", {
      challengesCount: newState.challenges?.length,
      submissionsCount: newState.tracker?.submissions?.length
    });
  }, 100);
}